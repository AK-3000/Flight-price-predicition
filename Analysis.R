...
ggplot(aes(y = Price, x = dep_time_slot, fill = dep_time_slot), data = train) + geom_boxplot()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+labs(title = "Boxplot of each time slot vs price")

#Plotting mean price of each dep_time_slot: 
ggplot(train, aes(x=dep_time_slot, y=Price, fill = Airline)) + stat_summary(fun.y="mean", geom="bar")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+labs(title = "Mean price of each airline")
...
#Plotting frequecy count of each airline:
ggplot(train1,aes(x=Airline,fill=Airline))+
  geom_bar(position="dodge")+labs(title = "Counts of each Airline")+
  geom_text(aes(label=..count..),stat='count',position=position_dodge(0.9),vjust=-0.2)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot(train1, aes(x=Source, y=Price, fill= Source)) + stat_summary(fun.y="mean", geom="bar")+
...
ggplot(train1, aes(Source, ..count..)) + geom_bar(aes(fill = Destination), position = "dodge")+labs(title = "Count of source vs Destination")
...
ggplot(train1,aes(x=train1$Airline,fill=train1$Source))+
  ...
  geom_text(aes(label=..count..),stat='count',position=position_dodge(0.9),vjust=-0.0)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

#Plotting mean price Vs Total_Stops:
ggplot(train1, aes(x=Total_Stops, y=Price, fill= Airline)) + stat_summary(fun.y="mean", geom="bar")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+labs(title = "Mean price vs total stops")

#Plotting mean price Vs duration:
ggplot(train1, aes(x=duration, y=Price, fill= duration)) + stat_summary(fun.y="mean", geom="bar")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+labs(title = "Mean price vs duration(hrs)")

#Plotting duration vs price: #important plot 
ggplot(train1, aes(x = duration, y = Price)) + geom_point()+ geom_smooth(method = "lm") 
...
ggplot(train1,aes(x=Airline,fill=dep_time_slot))+
...
scatter.smooth(x=train$Airline, y=train$Price, main="Dist ~ Speed")
...
train3=train3[, -c(1:4)]
train3=train3[,-c(3,4)]
train3$dep_time_slot_Pre_Morning=NULL
...
#DataExplorer::create_report(train3)
...
model = lm(formula = Price~ .,data = train3) 
y = as.data.frame(predict(model, newdata = test3))
View(y)
boxplot((y))
boxplot(y,
        main = "Predicted prices of Airline",
...
regressor = svm(formula = Price ~ .,data = train3,type = 'eps-regression',kernel = 'radial')
y_pred=predict(regressor,test3)
scatter.smooth(x=test$Airline, y=y_pred, main="Dist ~ Speed")
...
summary(model1)
summary(test)
ggplot(test, aes(x=test$Airline, y=test$Price)) + stat_summary(fun.y="mean", geom="bar")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+labs(title = "Mean price of each airline")
...
#RIDGE REGRESSION:
ridge=caret::train(Price~.,train3,method='glmnet',tuneGrid=expand.grid(alpha=0,lambda=seq(0.001,1,length=5)),
            trControl=custom)
...
model = caret::train(Price~ ., data= train3, trControl=train_control, method="nb")
model_cv<-train(Price ~ ., data = train3, method ='lm',trControl = ctrl,metric='Rsquared')
model = lm(formula = Price~ .,data = train3) 
y = as.data.frame(predict(model, newdata = test3))
# Summarise Results
